...
if [ -d "$TMP_DIR/artifacts" ]; then
    ...
    if [ -f "$TMP_DIR/artifacts/main-script-module.txt" ]; then
        cp "$TMP_DIR/artifacts/main-script-module.txt" "$INSTALL_DIR/main-script.sh"
        echo -e "${GREEN}Copied main script${NC}"
    elif [ -f "$TMP_DIR/artifacts/main-script.txt" ]; then
        cp "$TMP_DIR/artifacts/main-script.txt" "$INSTALL_DIR/main-script.sh"
        echo -e "${GREEN}Copied main script${NC}"
    else
        echo -e "${YELLOW}Looking for main script in artifacts...${NC}"
        MAIN_SCRIPT=$(find "$TMP_DIR/artifacts" -name "*main-script*.txt" | head -n 1)
    ...
    # Copy module files - core modules
    for module in core logging config error ui utils proton; do
        MODULE_FILE=$(find "$TMP_DIR/artifacts" -name "*$module-module*.txt" | head -n 1)
        if [ -n "$MODULE_FILE" ]; then
            cp "$MODULE_FILE" "$INSTALL_DIR/lib/$module.sh"
            echo -e "${GREEN}Copied $module module${NC}"
        else
    ...
    for module in common fallout enderal bg3; do
        MODULE_FILE=$(find "$TMP_DIR/artifacts" -name "*$module-module*.txt" -o -name "*games-$module*.txt" | head -n 1)
        if [ -n "$MODULE_FILE" ]; then
            cp "$MODULE_FILE" "$INSTALL_DIR/lib/games/$module.sh"
            echo -e "${GREEN}Copied games/$module module${NC}"
        else
            MODULE_FILE=$(find "$TMP_DIR/artifacts" -name "*$module*.txt" | head -n 1)
            if [ -n "$MODULE_FILE" ]; then
                cp "$MODULE_FILE" "$INSTALL_DIR/lib/games/$module.sh"
    ...
    for module in installation hoolamike; do
        MODULE_FILE=$(find "$TMP_DIR/artifacts" -name "*$module-module*.txt" -o -name "*ttw-$module*.txt" | head -n 1)
        if [ -n "$MODULE_FILE" ]; then
            cp "$MODULE_FILE" "$INSTALL_DIR/lib/ttw/$module.sh"
            echo -e "${GREEN}Copied ttw/$module module${NC}"
        else
            MODULE_FILE=$(find "$TMP_DIR/artifacts" -name "*$module*.txt" | head -n 1)
            if [ -n "$MODULE_FILE" ]; then
                cp "$MODULE_FILE" "$INSTALL_DIR/lib/ttw/$module.sh"
...
# Check for each module directly in the repository root
    ...
    # First, try to find the main script
