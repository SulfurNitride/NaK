...
class ModOrganizer2View(QWidget):
    ...
    def _create_options_area(self, parent_layout):
        ...
        self._create_3_plus_2_layout()
        
        parent_layout.addWidget(self.options_container)
        
    def _create_3_plus_2_layout(self):
        """Create the 3+2 button layout"""
        # Top row (3 buttons)
        top_row = QHBoxLayout()
        ...
        deps_button = self._create_option_button(
            "Install required dependencies for MO2",
            "Install Basic Dependencies",
            self._install_dependencies
        )
        top_row.addWidget(deps_button)
        ...
        # Add spacer to center the 2 buttons
        ...
        remove_button = self._create_option_button(
            ...
            "Remove MO2 NXM Handler",
            self._remove_nxm
        )
        bottom_row.addWidget(remove_button)
        
        bottom_row.addStretch()
        self.options_layout.addLayout(bottom_row)
    ...
    def _install_dependencies(self):
        """Show dependency installation submenu with game selection"""
        self.logger.info("Install dependencies clicked")
        ...
        # Show submenu with game selection
        self._show_dependency_install_view(games)
        
    def _show_dependency_install_view(self, games):
        """Show dependency installation submenu with game selection"""
        self._clear_content()
        self._create_dependency_install_view(games)
    
    def _create_dependency_install_view(self, games):
        """Create dependency installation submenu with game selection and NXM handler option"""
        # Title
        title_label = QLabel("Install Dependencies")
        title_font = QFont()
        title_font.setPointSize(24)
        title_font.setBold(True)
        title_label.setFont(title_font)
        ...
        # Instructions
        instructions_label = QLabel(f"Found {len(games)} non-Steam games. Please select one to install dependencies:")
        instructions_label.setStyleSheet("color: #b0bec5; font-size: 14px; margin-bottom: 20px;")
        instructions_label.setWordWrap(True)
        instructions_label.setAlignment(Qt.AlignCenter)
        self.content_layout.addWidget(instructions_label)
        ...
        """)
        self.game_list.setMaximumHeight(200)
        ...
        self.content_layout.addWidget(self.game_list)
        ...
        """)
        install_button.clicked.connect(self._install_dependencies_for_selected_game)
    ...
    def _install_dependencies_for_selected_game(self):
        ...
        # Show progress dialog
        progress = QProgressDialog(f"Installing dependencies for {game_name}...", "Cancel", 0, 0, self)
        progress.setWindowModality(Qt.WindowModal)
        progress.setAutoClose(False)
        progress.show()
        
        # Create worker thread
        self.worker = MO2WorkerThread("install_dependencies_for_game", core=self.controller.core, app_id=app_id)
        self.worker.finished.connect(lambda result: self._on_dependencies_finished(result, progress))
        self.worker.start()
    
    def _on_dependencies_finished(self, result, progress):
        ...
        if result.get("success"):
            ...
            msg_box.setIcon(QMessageBox.Information)
            msg_box.setWindowTitle("Success")
            msg_box.setText(f"Dependencies installed successfully!\n\n{result.get('message', '')}")
            msg_box.setStandardButtons(QMessageBox.Ok)
            msg_box.exec_()  # Use exec_() to make it modal
        else:
            # Show error dialog and wait for user to close it
            ...
            msg_box.setIcon(QMessageBox.Critical)
            msg_box.setWindowTitle("Error")
            msg_box.setText(f"Failed to install dependencies:\n{result.get('error', 'Unknown error')}")
            msg_box.setStandardButtons(QMessageBox.Ok)
            msg_box.exec_()  # Use exec_() to make it modal
    ...
    def _create_nxm_configure_view(self, games):
        ...
        self.manual_override_layout.addWidget(select_nxm_button)
