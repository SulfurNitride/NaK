...
try:
    ...
    VDF_AVAILABLE = False
    # Create a dummy vdf module for when it's not available
    class DummyVDF:
        @staticmethod
        def load(fp):
            return {}
...
class SteamShortcutManager:
    ...
    def generate_app_id(self, app_name: str, exe_path: str) -> int:
        """Generate a unique AppID for a non-Steam game"""
        import time
        import random

        # Include timestamp and random component to ensure uniqueness
        # even for same name+exe combinations
        timestamp = str(int(time.time() * 1000))  # milliseconds
        random_component = str(random.randint(1000, 9999))
        combined = exe_path + app_name + timestamp + random_component + "\x00"
        crc = zlib.crc32(combined.encode('utf-8'))
        # Match Go implementation format: crc | 0x80000000
    ...
    def list_shortcuts(self) -> List[SteamShortcut]:
        ...
        self.logger.debug(f"Looking for shortcuts at: {shortcuts_path}")

        if not os.path.exists(shortcuts_path):
            self.logger.warning(f"Shortcuts file does not exist: {shortcuts_path}")
            return []

        # Check file size
        file_size = os.path.getsize(shortcuts_path)
        self.logger.debug(f"Shortcuts file size: {file_size} bytes")

        try:
            with open(shortcuts_path, 'rb') as f:
                data = vdf.binary_load(f)

            self.logger.debug("VDF data loaded successfully")

            shortcuts = []
            shortcuts_data = data.get('shortcuts', {})

            self.logger.debug("Shortcuts data parsed from VDF")
            self.logger.info(f"Found {len(shortcuts_data)} existing shortcuts")
    ...
    def create_steam_shortcut(self, app_name: str, exe_path: str, proton_tool: str = "proton_experimental") -> int:
        ...
        self.logger.debug(f"Generated AppID: {app_id} (hex: {app_id:08x})")
    ...
    def add_single_shortcut(self, app_name: str, exe_path: str, proton_tool: str = "proton_experimental") -> int:
        ...
        # Always generate NEW unique AppID (like Steam does)
        app_id = self.generate_app_id(app_name, exe_path)
        self.logger.info(f"Creating new shortcut with AppID: {app_id}")

        # Prepare quoted paths
        exe_path_quoted = exe_path if exe_path.startswith('"') else f'"{exe_path}"'
        start_dir = str(Path(exe_path).parent)
        start_dir_quoted = start_dir if not start_dir or start_dir.startswith('"') else f'"{start_dir}"'
    ...
    def _write_shortcuts(self, shortcuts: List[SteamShortcut]):
        ...
        try:
            self.logger.debug(f"Writing {len(shortcuts)} shortcuts to VDF file")
            with open(shortcuts_path, 'wb') as f:
                vdf.binary_dump(data, f)
            self.logger.debug(f"Successfully wrote {len(shortcuts)} shortcuts")
        except Exception as e:
            self.logger.error(f"Failed to write shortcuts to VDF file: {e}")
            self.logger.debug(f"VDF write failed - data length: {len(str(data))} chars")
            raise
...
echo Prefix creation complete."""
        ...
        self.logger.debug(f"Creating initialization script: {bat_path}")
        ...
        self.logger.debug("Initialization script created")
        ...
        if not proton_path:
            self.logger.warning(" Proton Experimental not found")
    ...
    def _find_proton_installation(self, proton_version: str) -> Optional[str]:
        ...
        all_libraries = self._get_steam_libraries()
        self.logger.debug(f"Checking {len(all_libraries)} Steam libraries for Proton: {all_libraries}")
        ...
        for library_path in all_libraries:
            ...
            if common_path.exists():
                proton_path = common_path / proton_version / "proton"
                if proton_path.exists():
                    self.logger.info(f"Found Proton in Steam library: {proton_path}")
                    return str(proton_path)
    ...
    def _get_all_available_proton_versions(self) -> List[str]:
        """Get all available Proton versions on the system including Heroic"""
        versions = []
        home_dir = Path.home()
        ...
        for library_path in all_libraries:
            ...
            common_path = library_path / "steamapps" / "common"
            if common_path.exists():
                for entry in common_path.iterdir():
                    if entry.is_dir() and entry.name.startswith("Proton"):
                        proton_path = entry / "proton"
                        if proton_path.exists():
                            versions.append(entry.name)
                            self.logger.info(f"Found Proton in Steam library {library_path}: {entry.name}")

        # Check Heroic Proton installations
        heroic_proton_paths = [
            home_dir / ".config" / "heroic" / "tools" / "proton",
            home_dir / ".var" / "app" / "com.heroicgameslauncher.hgl" / "config" / "heroic" / "tools" / "proton",
            home_dir / "Games" / "Heroic" / "tools" / "proton"
        ]

        for heroic_proton_dir in heroic_proton_paths:
            if heroic_proton_dir.exists():
                for entry in heroic_proton_dir.iterdir():
                    if entry.is_dir() and (entry.name.startswith("Proton") or entry.name.startswith("GE-Proton")):
                        proton_path = entry / "proton"
                        if proton_path.exists():
                            versions.append(f"Heroic - {entry.name}")
                            self.logger.info(f"Found Heroic Proton: {entry.name}")
    ...
    def _detect_and_configure_games(self, compat_data_path: str):
        ...
        try:
            ...
            self.logger.debug(f"Searching {len(steam_libraries)} Steam libraries")
            ...
            self.logger.warning(f"Error during game detection: {e}")
    ...
    def _get_steam_libraries(self) -> List[Path]:
        ...
        try:
            ...
            self.logger.warning(f"Error reading Steam libraries: {e}")
                    ...
                    for line in content.split('\n'):
                        if '"installdir"' in line:
                            ...
                            if len(parts) >= 4:
                                ...
                                if game_path.exists():
                                    self.logger.debug(f"Found game via appmanifest: {game_path}")
                                    return game_path
                except Exception as e:
                    self.logger.warning(f"Error reading appmanifest for {app_id}: {e}")
            ...
            if common_path.exists():
                for name in common_names:
                    ...
                    if game_path.exists() and game_path.is_dir():
                        self.logger.debug(f"Found game via folder name: {game_path}")
                        return game_path
    ...
    def _modify_system_reg(self, compat_data_path: str, detected_games: list):
        ...
        if not system_reg_path.exists():
            self.logger.warning(f"system.reg not found at: {system_reg_path}")
            return
        
        self.logger.info(f"Updating registry for {len(detected_games)} games")
...
'''
                ...
                self.logger.debug(f"Added registry entry for {game['name']}")
            ...
            self.logger.info(f"Applied {total_entries} registry entries")
...
'''
    ...
    def find_steam_libraries(self) -> List[str]:
        ...
        try:
            ...
            self.logger.debug(f"Located {len(libraries)} Steam libraries")
            return libraries
    ...
    def create_steam_compat_mounts_string(self, libraries: List[str] = None) -> str:
        ...
        Includes both Steam libraries AND custom root directories
        """
        mount_paths = set()
        
        # 1. Add Steam libraries
        if libraries is None:
            libraries = self.find_steam_libraries()
        if libraries:
            mount_paths.update(libraries)
        
        # 2. Add non-standard root directories
        custom_dirs = self._detect_custom_root_directories()
        if custom_dirs:
            mount_paths.update(custom_dirs)
    ...
    def _detect_custom_root_directories(self) -> List[str]:
        """Detect non-standard directories in root /
        
        Returns directories that are NOT standard Linux system directories
        """
        try:
            ...
            self.logger.error(f"Failed to detect custom root directories: {e}")
            return []
