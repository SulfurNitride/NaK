...
class SteamUtils:
    ...
    def _initialize_shortcut_manager(self):
        ...
        try:
            ...
            self.logger.debug("Steam shortcut manager initialized")
    ...
    def get_non_steam_games(self) -> List[Dict[str, str]]:
        ...
        try:
            ...
            self.logger.error("This should not happen - VDF parsing is our primary method")
            return []
    
    def _get_games_via_protontricks(self) -> List[Dict[str, str]]:
        """Get all games (including non-Steam) using protontricks -l"""
        try:
            protontricks_cmd = self.get_protontricks_command()

            # Run protontricks -l to list all games
            cmd_list = protontricks_cmd.split() + ["-l"]
            self.logger.info(f"Running protontricks command: {' '.join(cmd_list)}")
            ...
            self.logger.debug(f"Protontricks output lines: {lines}")
            ...
            for line in lines:
                ...
                self.logger.debug(f"Processing protontricks line: {repr(line)}")

                # Parse protontricks output format: "Game Name (AppID)"
                # Example: "Fallout: New Vegas (22380)" or "Non-Steam Game Name (1234567890)"
                import re
                match = re.match(r'^(.+?)\s*\((\d+)\)$', line)
                if match:
                    game_name = match.group(1).strip()
                    app_id = match.group(2).strip()
                    self.logger.debug(f"Parsed AppID: {app_id}, Game: {game_name}")

                    # Add all games found by protontricks (both Steam and non-Steam)
                    # protontricks already filters to show relevant games for the user
                    try:
                        ...
                        self.logger.debug(f"Added game: {game_name} ({app_id})")
                    except ValueError as e:
                        # Skip lines that don't have valid AppIDs
                        self.logger.warning(f"Failed to parse AppID '{app_id}': {e}")
                        continue
                else:
                    self.logger.debug(f"Skipped line without valid format: {repr(line)}")

            self.logger.debug(f"Protontricks detected {len(games)} games")
            return games

        except subprocess.CalledProcessError as e:
            self.logger.error(f"protontricks command failed: {e}")
            raise
        except Exception as e:
            self.logger.error(f"Failed to parse protontricks output: {e}")
            raise

    def _get_non_steam_games_via_vdf(self) -> List[Dict[str, str]]:
        """Get non-Steam games by parsing VDF files (fallback method)"""
        if not self.shortcut_manager:
            self.logger.warning("Shortcut manager not available, using basic fallback")
        ...
        try:
            ...
            self.logger.error(f"Failed to get non-Steam games via VDF: {e}")
            return self._get_non_steam_games_fallback()
