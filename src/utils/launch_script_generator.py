...
'''
    ...
    def generate_unverum_launch_script(
        self,
        prefix_path: Path,
        unverum_exe_wine_path: str,
        instance_name: str,
        proton_ge_path: Optional[Path] = None
    ) -> Path:
        """
        Generate a launch script for Unverum

        Args:
            prefix_path: Path to the Wine prefix (e.g., ~/NaK/Prefixes/unverum_instance/pfx)
            unverum_exe_wine_path: Wine path to Unverum.exe (e.g., Z:\\path\\to\\Unverum\\Unverum.exe)
            instance_name: Display name for this instance
            proton_ge_path: Path to specific Proton-GE version (defaults to ~/NaK/ProtonGE/active)
        ...
        try:
            ...
            # Script will be placed in the parent directory of pfx
            script_dir = prefix_path.parent
            script_path = script_dir / "launch_unverum.sh"
...
PREFIX="{prefix_path_abs}"
COMPAT_DATA="{compat_data_path}"
UNVERUM_EXE="{unverum_exe_wine_path}"
STEAM_PATH="{steam_path}"
...
# Launch Unverum with Proton-GE
echo "Launching Unverum..."
echo "Instance: {instance_name}"
echo "Proton-GE: $PROTON_GE"
echo "Prefix: $PREFIX"
echo "Steam Path: $STEAM_PATH"

"$PROTON_GE/proton" run "$UNVERUM_EXE" "$@"
'''
            ...
            self.logger.info(f"Generated Unverum launch script: {script_path}")
            return script_path

        except Exception as e:
            self.logger.error(f"Failed to generate Unverum launch script: {e}")
            raise
