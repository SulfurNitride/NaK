...
echo "Creating Iced GUI AppImage..."
...
# Copy the Iced executable
if [ ! -f "target/release/nak-gui" ]; then
    echo "Iced app not found. Please build it first with ./build_iced.sh"
    exit 1
fi

cp target/release/nak-gui AppDir/usr/bin/nak-gui
chmod +x AppDir/usr/bin/nak-gui
...
if command -v appimagetool &> /dev/null; then
    ARCH=x86_64 appimagetool AppDir NaK-Linux-Modding-Helper-Iced.AppImage
    echo "AppImage created: NaK-Linux-Modding-Helper-Iced.AppImage"
